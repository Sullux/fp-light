...
const { assemblyCompiler } = require('./assemblyCompiler')
const { spreadCompiler } = require('./spreadCompiler')
const { rangeCompiler } = require('./rangeCompiler')
const { dereferenceCompiler } = require('./dereferenceCompiler')
const { assignCompiler } = require('./assignCompiler')
const { passScopeCompiler } = require('./passScopeCompiler')
const { pipeScopeCompiler } = require('./pipeScopeCompiler')
const { passCompiler } = require('./passCompiler')
const { pipeCompiler } = require('./pipeCompiler')
const { labelCompiler } = require('./labelCompiler')
const { functionCompiler } = require('./functionCompiler')
const { scopeCompiler } = require('./scopeCompiler')
const { inlineCompiler } = require('./inlineCompiler')
const { moduleCompiler } = require('./moduleCompiler')
const { pragmaCompiler } = require('./pragmaCompiler')
...
const defaultContext = {
  ...
  parsers: [
    ...
    parseStraddle('assign', ':'),
    parseStraddle('passScope', '\\\\'),
    parseStraddle('pipeScope', '||'),
    parseStraddle('pass', '\\'),
    parseStraddle('pipe', '|'),
    parseLabel,
    parseString('"', 'label'),
  ...
  stack: [],
  compilers: {
    number: ({ element: { value } }) => ({ type: Number.isInteger(value) ? 'int' : 'float', value }),
    text: ({ element: { value } }) => ({ type: 'string', value }),
    assembly: assemblyCompiler,
    spread: spreadCompiler,
    range: rangeCompiler,
    dereference: dereferenceCompiler,
    assign: assignCompiler,
    passScope: passScopeCompiler,
    pipeScope: pipeScopeCompiler,
    pass: passCompiler,
    pipe: pipeCompiler,
    label: labelCompiler,
