...
describe('set', () => {
  ...
  it('should be async-aware on input', () => 
    set(['foo', 1], 42, Promise.resolve({ foo: [] }))
      .then(result => deepStrictEqual(result, { foo: [undefined, 42] })))
  it('should be async-aware on nested', () => 
    set(['foo', 1], 42, { foo: Promise.resolve([]) })
      .then(result => deepStrictEqual(result, { foo: [undefined, 42] })))
  it('should set a value in a map', () =>
    deepStrictEqual(
      [...set('foo', 42, new Map([['bar', 'baz']]))],
      [['bar', 'baz'], ['foo', 42]]
    ))
})
