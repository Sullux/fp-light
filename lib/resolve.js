...
const spreadableProxy = {
  ...
  ownKeys: (target) => target[compiledFrom]
    ? distinct([...Reflect.ownKeys(target).filter(key => key !== compiledFrom && key !== '$'), spreadableSymbol(), Symbol.iterator])
...
const identityProxy = {
  get: (target, property) => {
    if (property === Symbol.toPrimitive) {
      return () => target[primitiveFunctionSymbol] || pushPrimitiveFunction(target)
    }
    ...
    : isSpreadableSymbol(property)
      ? target
    : (property === 'then') || (property === 'constructor')
      ? target[property]
...
export const delay = compilable(function delay (ms, result) {
  return awaitDelay(ms).then(() => result)
})
