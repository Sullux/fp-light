...
const targetPropertyIfExistsOrInContext = (target, property) =>
  (typeof property === 'function')
    ? property
    : function targetPropertyIfExistsOrInContext(input) {
      const value = target(input)
      if (!value) { return }
...
const targetPropertyIfExistsOnlyInContext = (target, property) =>
  (typeof property === 'function')
    ? property
    : function targetPropertyIfExistsOnlyInContext() {
...
const passthroughProperties = [
  ...
  Symbol.toPrimitive,
  Symbol.for('fp.sync'),
  Symbol.for('fp.deepAwait'),
  Symbol.for('fp.isDeepSync'),
  'then',
  'constructor',
  'prototype',
  'call',
...
const propertyResolverProxy = (propertyResolver) => ({
  get: (target, property) => {
    ...
    if (accessor) {
      return new Proxy(input => target(input)[accessor(input)], thisIdentityProxy)
    }
        ...
        : (property === arraySpread)
          ...
          : property === Symbol.iterator
            ...
            : isSpreadableSymbol(property)
              ...
              : property === '$'
                ? target
                : passthroughProperties.includes(property)
                  ? target[property]
                  : new Proxy(
                    propertyResolver(target, property),
                    thisIdentityProxy,
                  )
  },
  getOwnPropertyDescriptor: (target, property) =>
...
const identityProxy = propertyResolverProxy(targetPropertyIfExistsOrInContext)
const baseIdentityProxy = propertyResolverProxy(targetPropertyIfExistsOnlyInContext)
const thisIdentityProxy = propertyResolverProxy(targetPropertyIfExists)
...
export const thisIdentity = new Proxy(
  function thisIdentity(v) { return v },
  thisIdentityProxy,
)
...
const settleCompilableArg = (arg) =>
  (arg && arg[compiledFrom] && arg()) || arg
    ...
    return (property === Symbol.isConcatSpreadable)
      ...
      : (property === arraySpread)
        ...
        : property === Symbol.iterator
          ...
          : isSpreadableSymbol(property)
            ? fn
            : ((property === compiledFrom) || passthroughProperties.includes(property))
              ? undefined
...
const compiledLiteral = (fn, arg) => {
  const apply = (target, thisArg, args) => context.enter(args[0], () => {
    const resolved = deepAwait(args)
    if (isAsync(resolved)) {
      return context.await(resolved, (resolvedArgs) =>
        fn.apply(resolvedArgs[0], [arg, ...resolvedArgs]))
    }
...
const compiledWithPasshrough = (fn, passthroughArgs, resolver) => {
  const apply = (target, thisArg, [arg]) => context.enter(arg, () => {
...
export const compilable = (fn, { count, skip = 0 } = {}) =>
  toSpreadable(override({
    properties: { '$': fn, [compiledFrom]: fn },
    apply: (target, thisArg, args) => context.enter(
      { target, thisArg, args },
      () => {
        if (args.length === 0) {
