import { deepAwait, isAsync } from './core/async.js'
import { compilable, resolve } from './resolve.js'
import { override } from './proxy.js'
import { compare, isIterable } from './core/compare.js'
...
export const reduce = compilable(function reduce (
  ...
  const reduceSync = (input) => {
    ...
    const reduceAsync = async () => {
      for (; i < length;) {
        state = await context.await(reducer, {
          state,
          value: input[i],
          i,
          array: input,
    ...
    for (; i < length;) {
      ...
      if (isAsync(state)) {
        return context.await(state, startMappingAsync)
      }
      i++
    }
    return state
  ...
  return isAsync(maybeAsyncInput)
    ? context.await(maybeAsyncInput, reduceSync)
...
function joinLiteral ({
  ...
  if (isAsync(resolved)) {
    return resolved.then(inputs =>
      joinLiteral({ ...inputs, on, map, outer }))
  }
  ...
  return left.reduce(
    (state, value) => {
      let matched
      right.filter(v => compared({ left: value, right: v }))
...
export const sort = compilable(function sort (comparer, array) {
  ...
  return isAsync(result)
    ...
    : result.sort(comparer || defaultComparer)
}, { skip: 1 })
