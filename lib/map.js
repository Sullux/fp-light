...
export const reduce = compilable(function reduce ({ state: currentState, reducer }, array) {
  let state = currentState
  let i = 0
...
function joinLiteral ({left, right, on = defaultJoinPredicate, map }) {
  ...
  const mapper = map && resolve(map)
  const mutate = mapper
    ? (state, left, right) => state.push (mapper({ left, right }))
  ...
  return reduce.$(
    {
      ...
      reducer: ({ state, value }) => {
        rightArray.filter(v => compared({ left: value, right: v }))
          .forEach(match => mutate(state, value, match))
        return state
      },
    },
    left,
