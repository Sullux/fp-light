...
export const reduce = compilable(function reduce ({ state: currentState, reducer: rawReducer }, array) {
  ...
  for (const value of iterable) {
    ...
    // console.log('***', { state, value, i, array })
    state = reducer(base({ state, value, i, array }))
    i++
  }
  return state
}, { skip: 1 })
...
function joinLiteral ({left, right, on = defaultJoinPredicate, map, outer, base }) {
  const resolved = deepAwait({ left, right })
  if (isAsync(resolved)) {
    return resolved.then(inputs => joinLiteral({ ...inputs, on, map, outer, base }))
  }
  const compared = resolve(on)
  const rightArray = toArray(right)
  if (rightArray.some(isAsync)) {
    return toSyncArray(rightArray)
      .then(resolvedArray => joinLiteral({
        left,
        right: resolvedArray,
        on,
        map,
  ...
  return reduce.$(
    {
      ...
      reducer: ({ state, value }) => {
        let matched
        rightArray.filter(v => compared(base({ left: value, right: v })))
