...
const { isFunction, isString, functionName } = require('./helpers')
...
const defaultResolve = (parts, args) => {
  let result = args[0]
...
function Identity (fn, parts = [], resolve = defaultResolve) {
  ...
  const apply = (target, thisArg, args) => resolve(parts, args)
...
const spreadToArray = (identity, count) =>
  Array(count).fill().map((x, i) => identity[i])
const spreadToObject = (identity, names) =>
  Object.fromEntries(names.map((name) => ([name, identity[name]])))
Identity.spread = (identity, ...specification) =>
  isString(specification[0])
    ? spreadToObject(identity, specification)
    : spreadToArray(identity, specification[0])
