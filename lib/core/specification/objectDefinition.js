...
const factory = (Specification) => {
  const objectsAreCompatible = (o1, o2) => {
    const leftSpread = o1.filter(([key]) => Array.isArray(key))
      .map(([[kType, kOptions], [vType, vOptions]]) => ([
        Specification.for(kType)(kOptions),
        Specification.for(vType)(vOptions),
      ]))
    const rightSpread = o2.filter(([key]) => Array.isArray(key))
      .map(([[kType, kOptions], [vType, vOptions]]) => ([
        Specification.for(kType)(kOptions),
        [vType, vOptions],
      ]))
    const leftNamed = o1.filter(([key]) => !Array.isArray(key))
    const rightNamed = o2.filter(([key]) => !Array.isArray(key))
  ...
  const objectIsImplemented = (options, v) => {
    ...
    const spread = options.filter(([key]) => Array.isArray(key))
      .map(([[kType, kOptions], [vType, vOptions]]) => ([
