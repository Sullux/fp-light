import { awaitAll, isThennable } from './async'
...
const callIterable = (predicate) => {
  const calls = (Array.isArray(predicate) ? predicate : [...predicate])
    .map(c => (c && c[arraySpread]) ? c : call(c))
  return input => {
    const result = calls.reduce(
      (result, c) => {
        const callToSpread = c && c[arraySpread]
        const getSpreadValues = () => {
          const values = iterableToArray(callToSpread(input))
          return values.some(isThennable)
            ? awaitAll(values)
            : values
        }
...
const callProxy = (predicate, call) => {
  const overrides = { '$call': predicate }
...
export const isCall = predicate =>
  ((typeof predicate) === 'function') && ('$call' in predicate)
...
description: |
  ...
  It is a curried function that accepts a _call predicate_ and an input value. A
  _call predicate_ is one of:
  ...
  * a promise resolving to any of the above.
  ...
  Additionally, a call predicate can include or return a promise or a value that
...
examples: |
  ...
  values.map(call(_.foo))
  // [41, 42, 43]

  // remap the foo element to bar for each element

  values.map(call({ bar: _.foo }))
  // [
  //   { bar: 41 },
  //   { bar: 42 },
  //   { bar: 43 },
...
*/
export const call = (predicate) =>
  callProxy(predicate, isConstant(predicate)
    ? () => predicate
    : typeof predicate === 'function'
      ? predicate
      : typeof predicate === 'object'
        ? predicate[Symbol.iterator]
          ? callIterable(predicate)
          : callObject(predicate)
        : () => predicate)

const callableProxy = (fn, callable) => {
  const overrides = { '$': fn }
  return new Proxy(fn, {
    get: (target, prop) =>
      overrides[prop] || target[prop],
...
export const callable = fn => {
  const callable = (...args) => {
    const callableArgs = args.map(call)
    return value => {
      const results = callableArgs.map(c => c(value))
      return results.some(isThennable)
        ? awaitAll(isThennable).then(resolvedResults => fn(...resolvedResults))
        : fn(...results)
    }
  }
  return trace(callableProxy(fn, callable))
}

export { callable as toCallable }
